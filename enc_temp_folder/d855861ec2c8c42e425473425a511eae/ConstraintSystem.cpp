                       ...
                       const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 3) {
        ...
        glm::dvec3 projectedPoint = MathUtils::projectPointOnPlane(inputVec, normal, p1);
                      ...
                      const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 2) {
        glm::dvec3 lineStart = glm::dvec3(points[0].x(), points[0].y(), points[0].z());
        glm::dvec3 lineEnd = glm::dvec3(points[1].x(), points[1].y(), points[1].z());
        
        // 投影输入点到直线
        glm::dvec3 inputVec = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        glm::dvec3 projectedPoint = MathUtils::projectPointOnLine(inputVec, lineStart, lineEnd);
                                ...
                                const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 3) {
        ...
        for (const auto& point : points) {
            basePoints.push_back(glm::dvec3(point.x(), point.y(), point.z()));
        }
        
        glm::dvec3 baseCenter = MathUtils::calculateCentroid(basePoints);
        glm::dvec3 baseNormal = MathUtils::calculatePolygonNormal(basePoints);
        
        // 计算输入点在垂直于底面的直线上的投影
        glm::dvec3 inputVec = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        
        // 垂直线的方向就是底面法向量
        glm::dvec3 verticalDirection = baseNormal;
        
        // 计算投影：input点到垂直起点的向量在法向量上的投影
        glm::dvec3 toInput = inputVec - baseCenter;
        double projectionLength = glm::dot(toInput, verticalDirection);
        
        // 约束后的点 = 垂直起点 + 投影长度 * 法向量
        glm::dvec3 constrainedPoint = baseCenter + projectionLength * verticalDirection;
                                              ...
                                              const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 2) {
        ...
        // 计算向量AB和BC
        glm::dvec3 vecA = glm::dvec3(pointA.x(), pointA.y(), pointA.z());
        glm::dvec3 vecB = glm::dvec3(pointB.x(), pointB.y(), pointB.z());
        glm::dvec3 vecC = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        
        glm::dvec3 AB = vecB - vecA;
        ...
        // 计算BC在AB方向上的投影
        glm::dvec3 ABNormalized = glm::normalize(AB);
        double projectionLength = glm::dot(BC, ABNormalized);
        glm::dvec3 projectionOnAB = projectionLength * ABNormalized;
                        ...
                        const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 2) {
        ...
        // 计算从A到输入点的向量
        glm::dvec3 pointAVec = glm::dvec3(pointA.x(), pointA.y(), pointA.z());
        glm::dvec3 inputVec = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        glm::dvec3 toInput = inputVec - pointAVec;
        
        // 如果输入点就在A点，返回B点
        if (glm::length(toInput) < 1e-6) {
            return pointB;
        }
        
        // 将输入点约束到以A为圆心、|AB|为半径的球面上
        // 保持输入点相对于A的方向，但距离调整为|AB|
        glm::dvec3 normalizedDirection = glm::normalize(toInput);
        glm::dvec3 constrainedPoint = pointAVec + normalizedDirection * targetRadius;
                                             ...
                                             const std::vector<Point3D>& points)
{
    ...
    if (points.size() >= 3) {
        ...
        // 转换为glm向量进行计算
        glm::dvec3 centerVec = glm::dvec3(circleCenter.x(), circleCenter.y(), circleCenter.z());
        glm::dvec3 bVec = glm::dvec3(pointB.x(), pointB.y(), pointB.z());
        glm::dvec3 cVec = glm::dvec3(pointC.x(), pointC.y(), pointC.z());
        glm::dvec3 inputVec = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        
        // 计算圆平面的两个向量
        glm::dvec3 vectorAB = bVec - centerVec;
        glm::dvec3 vectorAC = cVec - centerVec;
        
        // 计算圆平面的法向量
        glm::dvec3 normal = glm::cross(vectorAB, vectorAC);
        ...
        // 计算从圆心到输入点的向量
        glm::dvec3 toInput = inputVec - centerVec;
        
        // 计算输入点在法向量方向上的投影长度
        double projectionLength = glm::dot(toInput, normal);
        
        // 约束后的点 = 圆心 + 投影长度 * 法向量
        glm::dvec3 constrainedPoint = centerVec + projectionLength * normal;
...
Point3D equalLengthConstraint(const Point3D& inputPoint, 
                             const std::vector<Point3D>& points)
{
    // 检查是否有至少3个点：A、B构成参考线段，C是新线段的起点
    if (points.size() >= 3) {
        Point3D pointA = points[0];     // 参考线段起点A
        Point3D pointB = points[1];     // 参考线段终点B
        Point3D pointC = points[2];     // 新线段起点C
        
        // 计算参考线段AB的长度
        double referenceLength = sqrt(
            (pointB.x() - pointA.x()) * (pointB.x() - pointA.x()) +
            (pointB.y() - pointA.y()) * (pointB.y() - pointA.y()) +
            (pointB.z() - pointA.z()) * (pointB.z() - pointA.z())
        );
        ...
        // 计算从C到输入点的向量
        glm::dvec3 pointCVec = glm::dvec3(pointC.x(), pointC.y(), pointC.z());
        glm::dvec3 inputVec = glm::dvec3(inputPoint.x(), inputPoint.y(), inputPoint.z());
        glm::dvec3 toInput = inputVec - pointCVec;
        
        // 如果输入点就在C点，返回沿任意方向距离为referenceLength的点
        if (glm::length(toInput) < 1e-6) {
            // 默认沿X轴正方向
            glm::dvec3 constrainedPoint = pointCVec + glm::dvec3(referenceLength, 0.0, 0.0);
            return Point3D(constrainedPoint.x, constrainedPoint.y, constrainedPoint.z);
        }
        
        // 将输入点约束到以C为圆心、referenceLength为半径的球面上
        // 保持输入点相对于C的方向，但距离调整为referenceLength
        glm::dvec3 normalizedDirection = glm::normalize(toInput);
        glm::dvec3 constrainedPoint = pointCVec + normalizedDirection * referenceLength;
