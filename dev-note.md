...
cmake尝试了好久(VS使用vcpkg可以直接用，不知道为什么能找到osgQOpenGL.lib)，主要是osgqt的cmake问题，试出来一种可能的方案

可以在麒麟上运行（build.sh,使用ninja）



#### 2025.7.7

加了天空盒和坐标系统（依然AI）所以都比较独立，效果上不好，需要自己结合写（只能说坑越来越多）
...
因为要绘制点、线、面，体以及他们的组合，线串、复杂面啥的。

原来的通过控制点外加描述符的方式感觉很难描述了。
