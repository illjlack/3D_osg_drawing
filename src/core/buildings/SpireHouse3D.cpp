...
void SpireHouse3D_Geo::buildVertexGeometries()
{
    ...
    if (controlPoints.size() >= 3)
    {
        ...
        m_spireHeight = m_size.z * 0.4f;  // 尖顶高度为房屋高度的40%
        m_spireRadius = std::min(m_size.x, m_size.y) * 0.1f;  // 尖顶半径为较小边长的10%
        ...
        // 尖顶顶点
        float spireX = x + m_size.x * 0.5f;
        float spireY = y + m_size.y * 0.5f;
        float spireZ = z + m_size.z + m_spireHeight;
        vertices->push_back(osg::Vec3(spireX, spireY, spireZ));
    }
...
void SpireHouse3D_Geo::buildEdgeGeometries()
{
    ...
    if (controlPoints.size() >= 3)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            osg::ref_ptr<osg::DrawArrays> spireLine = new osg::DrawArrays(osg::PrimitiveSet::LINES, 4 + i, 2);
            geometry->addPrimitiveSet(spireLine);
        }
    }
}
...
void SpireHouse3D_Geo::buildFaceGeometries()
{
    ...
    if (controlPoints.size() >= 3)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            osg::ref_ptr<osg::DrawArrays> spireFace = new osg::DrawArrays(osg::PrimitiveSet::TRIANGLES, 4 + i, 3);
            geometry->addPrimitiveSet(spireFace);
        }
    }
}
